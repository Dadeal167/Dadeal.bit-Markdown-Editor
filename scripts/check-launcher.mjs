...
function runBat(name, env = {}) {
  return new Promise((done) => {
    const child = spawn('cmd.exe', ['/d', '/c', resolve(LAUNCHER, name)], {
      cwd: FOLDER,
...
const LAUNCHER = resolve(FOLDER, '启动器')
const START = resolve(LAUNCHER, '开始使用.bat')
const STOP = resolve(LAUNCHER, '停止助手.bat')
...
check('有「安装助手自启 autostart-on.bat」', existsSync(resolve(LAUNCHER, '安装助手自启 autostart-on.bat')))
check('有「取消助手自启 autostart-off.bat」', existsSync(resolve(LAUNCHER, '取消助手自启 autostart-off.bat')))
...
/* 目录整洁：.bat 全部收进「启动器」子文件夹，根目录只留编辑器 + 说明 + 两个数据文件夹
   （原来根目录混着 5 个 .bat 和 _open-editor.ps1，用户看不出该点哪个） */
{
  const stray = existsSync(FOLDER) ? readdirSync(FOLDER).filter((f) => /\.(bat|ps1)$/i.test(f)) : ['(文件夹不存在)']
  check('根目录没有散落的 .bat / .ps1（都收进「启动器」）', stray.length === 0, stray.join('、') || '干净')
}
...
{
  const manual = resolve(LAUNCHER, '启动知乎助手.bat')
