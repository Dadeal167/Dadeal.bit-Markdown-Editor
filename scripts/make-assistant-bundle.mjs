...
exit /b 1
`
writeFileSync(resolve(STAGE, '启动知乎助手.bat'), ASSIST_BAT.replace(/\n/g, '\r\n'), 'ascii')
...
2. 双击「开始使用.bat」——它会在后台悄悄启动助手（不弹黑窗口）、自动带上令牌、并打开编辑器
3. 编辑器里点工具栏「知乎」，看到「✅ 助手已连接」就能用了
4. 写完后点「存到草稿箱」：它只存草稿、**不会发布**，你到知乎里接着改、确认没问题再自己发

【不想每次都双击？】
双击一次「安装助手自启 autostart-on.bat」，以后开机它就在后台静默跑着；
不想要了就双击「取消助手自启 autostart-off.bat」。想临时关掉：双击「停止助手.bat」。

【出问题怎么办】
· 「开始使用.bat」报错 → 改双击「启动知乎助手.bat」，它会把窗口留着，里面能看到原因
