...
/* "DOM 里数到有图了" ≠ "知乎服务器存下了"。
   不等它就会出现"8 张都插进去了，刷新后草稿里只有 7 张"（实测遇到过好几轮）。 */
check(
  '每插一张图都要"触发保存 + 等它真存完"（光看「刚刚」会被上一次保存骗过去）',
  /async function nudgeAndWaitSaved/.test(SRC) &&
    /keyboard\.type\(\s*' '\s*\)/.test(SRC) &&
    /保存中/.test(SRC) &&
    (() => {
      const from = SRC.indexOf('async function pasteViaClipboard')
      const to = SRC.indexOf('async function insertImageDirect')
      const body = from >= 0 && to > from ? SRC.slice(from, to) : ''
      return /await nudgeAndWaitSaved\(page\)/.test(body)
    })(),
)
/* 位置标记是内部占位符，绝不该留在用户草稿里（实测出现过残留文字）。
   收尾要清一遍，并且复核里要数残留数。 */
check(
  '收尾清理残留的位置标记 + 复核里检查残留数',
  /const cutMarker = async/.test(SRC) && /markersLeft/.test(SRC) && /DADEALBITIMG\\d\+MARK/.test(SRC),
)
/* 只数图片个数看不出位置对不对（用户报过"图全跑到文末"）。
   复核里要带"草稿骨架"：按 Draft.js 的块顺序把段落和图片列出来。 */
check(
  '复核里带"草稿骨架"（段落/图片按顺序），能看出图有没有插在原位',
  /data-block="true"/.test(SRC) && /outline,/.test(SRC) && /草稿骨架/.test(SRC),
)
...
/* 自检按钮坏过的两个原因（2026-09-22 实测踩到、都修了）：
   1. 标题框只认 <textarea>，而知乎的标题现在是 <input> → 30 秒超时
   2. reuseExistingDraft 找不到同名草稿时页面停在"草稿列表页"就返回
      → 自检接着在列表页上找标题框，必然超时（表现为"自检永远没通过"） */
check(
  '自检用和正式上传一样的宽松标题选择器（textarea 或 input 都认）',
  /titleBox = page\.locator\('textarea\[placeholder\*="标题"\], input\[placeholder\*="标题"\]'\)/.test(SRC),
)
check(
  '找不到同名草稿时把页面带回写作页（否则自检会停在草稿列表页超时）',
  /if \(!href\) \{[\s\S]{0,400}?await page\.goto\(WRITE_URL/.test(SRC),
)
check(
