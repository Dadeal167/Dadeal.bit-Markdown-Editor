...
/* ---------- 3. 本地数据库真的写不进去时：要提示、不崩、状态栏常驻提醒 ----------
   注意（2026-09 改动）：文档现在存在 **IndexedDB**（localStorage 只有 5MB，
   装不下带图的文档；老数据启动时自动迁过去）。所以"存不下"要改成让 IndexedDB 写失败，
   不再是往 localStorage 里塞垃圾。 */
await page.evaluate(() => {
  window.__alerts = []
  window.alert = (m) => window.__alerts.push(String(m))
  // 让所有 IndexedDB 写入当场抛配额错误（模拟磁盘满 / 浏览器不让存）
  window.__origPut = IDBObjectStore.prototype.put
  IDBObjectStore.prototype.put = function () {
    throw new DOMException('simulated quota', 'QuotaExceededError')
  }
})
record('测试前提：已经让 IndexedDB 写入必定失败', await page.evaluate(() => IDBObjectStore.prototype.put !== window.__origPut))
...
record('写不进去之后不再反复弹窗', secondAlerts === 1, `累计弹窗 ${secondAlerts} 次`)

// 把 IndexedDB 的写入能力还原：应当自动恢复正常保存
await page.evaluate(() => {
