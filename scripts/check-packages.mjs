...
/* ============================================================
 * C. 交付文件夹里的 HTML 必须与仓库产物**字节级相同**
 *
 *    为什么要这条（2026-09-26 真事故）：
 *    用户报"导入正常 → 刷新出方框 □ → 再刷新 \ 变 \\"的三步降级。
 *    查了很久才发现：**桌面那份是旧的构建**（12:41 的，onUpdate 收尾修复还没进去），
 *    里面"标记守卫"这个字符串**在** —— 所以我"查字符串在不在"的验证法给了假绿；
 *    连 `git hash-object` 比对也给了错的"一致"结论。
 *    真相是 `Get-FileHash`/SHA256 一比：2964393 字节 vs 2964397 字节，根本不是同一个文件。
 *
 *    教训：验"产物是不是最新"必须用**字节级指标**，不能查特征字符串
 *   （旧版往往也有那个字符串）。这条断言把这件事钉死。
 * ============================================================ */
{
  const sha = (p) => {
    try {
  ...
  /* 找仓库根：从脚本所在目录往上找带 package.json 的那层 */
  let root = resolve(import.meta.dirname, '..')
  for (let i = 0; i < 4 && !existsSync(resolve(root, 'package.json')); i += 1) {
    root = resolve(root, '..')
  }
  const repoPure = resolve(root, 'Dadealbit Markdown 编辑器.html')
  const repoEffect = resolve(root, 'Dadealbit Markdown 编辑器-特效版.html')

  const pureSame = existsSync(repoPure) && sha(PUB) === sha(repoPure)
  check(
    'C1 分享包里的 HTML 与仓库纯净版字节级相同（不是旧构建）',
    pureSame,
    pureSame
      ? `${Math.round(readFileSync(PUB).length / 1024)} KB 一致`
      : `桌面 ${sha(PUB)?.slice(0, 12) ?? '缺失'} vs 仓库 ${sha(repoPure)?.slice(0, 12) ?? '缺失'} —— 桌面那份是旧构建，跑 pnpm pack:folders 重新打包`,
  )
  const mineSame = existsSync(repoEffect) && sha(MINE) === sha(repoEffect)
  check(
    'C2 自用包里的 HTML 与仓库特效版字节级相同（不是旧构建）',
    mineSame,
    mineSame
      ? `${Math.round(readFileSync(MINE).length / 1024)} KB 一致`
      : `桌面 ${sha(MINE)?.slice(0, 12) ?? '缺失'} vs 仓库 ${sha(repoEffect)?.slice(0, 12) ?? '缺失'} —— 桌面那份是旧构建，跑 pnpm pack:folders 重新打包`,
  )
}
