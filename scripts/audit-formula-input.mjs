...
/* 8. 候选列表变短后按 Enter 不能崩（修过的坑）
   以前：输入 `\sq` 后按 ↓ 选到第 2 条 → 再点分类面板里的符号（tex 变了、候选变短）
   → 按 Enter 时 `suggestions[acIndex]` 是 undefined → s.latex 抛 TypeError，弹窗卡住。 */
await openMath()
await setTexAndCaret('\\sq')
await page.waitForTimeout(300)
const shortBefore = await items().count()
await page.keyboard.press('ArrowDown')
await page.waitForTimeout(150)
// 点一个符号按钮：tex 被改写、候选列表跟着变短
...
const survived = await page.evaluate(() => {
  const ta = document.querySelector('.zh-mathsource')
  return { tex: ta ? ta.value : null, modal: !!document.querySelector('.zh-modal--math') }
})
record(
  '候选变短后按 Enter/Tab 不崩（弹窗还在、tex 没坏）',
  errors.length === errBefore && survived.modal && typeof survived.tex === 'string',
  `候选 ${shortBefore}→点符号后，tex=${JSON.stringify((survived.tex ?? '').slice(0, 24))}，新报错 ${errors.length - errBefore} 条`,
)
await page.keyboard.press('Escape')
await page.waitForTimeout(250)
