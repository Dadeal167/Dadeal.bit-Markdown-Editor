...
（github.com/Dadeal167/Dadeal.bit-Markdown-Editor/releases/latest），解压后双击里面的「开始使用.bat」，
...
4. 点「存到草稿箱」，等它跑完（一般十几秒；图片多时知乎要多花几秒做托管）。
   速度参考：整篇一次交完，公式一百个也不慢；跑完面板会报「公式 N 个 · 图片 N/N 张」——
   数字对不上就别急着用，先去草稿箱看一眼。
...
· 上传过程不占用你的剪贴板，也不会弹浏览器窗口（有图片也是后台做完）。
