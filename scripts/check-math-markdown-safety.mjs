...
/* ============================================================
 * J：控制字符绝不许出现在导出 / 复制的结果里
 *
 *    真实事故：用户把编辑器内容 Ctrl+C 粘到记事本，**每个公式的定界符都变成了 U+0002**
 *    （他那边显示成"豆腐块" □，另一次粘贴里完全不可见 → 看起来就是"$ 全丢了、LaTeX 裸奔"）。
 *    逐字节取证他的文件：U+0002 出现 **332 次**，而那篇文档正好 **332 个 `$`** —— 精确吻合，
 *    说明 MATH_INLINE_MARK 流出去了。但我复现不出那条路径（程序化 / 键盘 / 真实 Ctrl+C
 *    三种入口都测过，都没泄漏），所以这条防线**不依赖复现**：直接扫结果。
 *
 *    这正是用户提的那条断言："文档解析完成后，正文里不允许出现任何控制字符"。
 * ============================================================ */
console.log('\n=== J. 控制字符绝不许出现在导出/复制结果里 ===')

const CTRL_RE = /[\u0000-\u0008\u000b\u000c\u000e-\u001f]/g

const leak = await page.evaluate(() => {
  const ed = window.__EDITOR__
  window.__SET_MARKDOWN__(ed, '前面 $\\left| TF_{1} \\right|=4$ 中间 $x^{2}*{n}$ 后面')
  const exported = window.__MD__()
  ed.commands.selectAll()
  const dt = new DataTransfer()
  ed.view.dom.dispatchEvent(new ClipboardEvent('copy', { clipboardData: dt, bubbles: true, cancelable: true }))
  return { exported, plain: dt.getData('text/plain') }
})
check('J1 导出 .md 里没有控制字符', !CTRL_RE.test(leak.exported), `命中 ${(leak.exported.match(CTRL_RE) || []).length} 个`)
check('J2 复制成纯文本时没有控制字符', !CTRL_RE.test(leak.plain), `命中 ${(leak.plain.match(CTRL_RE) || []).length} 个`)
check(
  'J3 导出/复制的 $ 个数正确（2 个公式 → 4 个 $）',
  (leak.exported.match(/\$/g) || []).length === 4 && (leak.plain.match(/\$/g) || []).length === 4,
  `导出 ${(leak.exported.match(/\$/g) || []).length} / 复制 ${(leak.plain.match(/\$/g) || []).length}`,
)

/* 守卫本体：直接喂"用户那种泄漏形态"，看它能不能清干净且不丢公式 */
const hasGuardHook = await page.evaluate(() => typeof window.__STRIP_LEAKED_MARKS__ === 'function')
check('J4 守卫本体可用（__STRIP_LEAKED_MARKS__）', hasGuardHook)
if (hasGuardHook) {
  const g = await page.evaluate(() => {
    const leaked =
      '已知点 \u0002F_{1}(-1,0),F_{2}(1,0),\u0002 动点 \u0002T\u0002 满足 \u0002\\left| TF_{1} \\right|=4,\u0002 尾'
    const once = window.__STRIP_LEAKED_MARKS__(leaked, 'test')
    const twice = window.__STRIP_LEAKED_MARKS__(once, 'test')
    return { once, twice }
  })
  check('J5 守卫把控制字符清干净', !CTRL_RE.test(g.once), `残留 ${(g.once.match(CTRL_RE) || []).length} 个`)
  check(
    'J6 守卫把成对标记还原成 $（公式没丢）',
    (g.once.match(/\$/g) || []).length === 6,
    `还原出 ${(g.once.match(/\$/g) || []).length} 个 $（应为 6）`,
  )
  check('J7 守卫幂等', g.once === g.twice, g.once === g.twice ? '' : '两次结果不同')
  check(
    'J8 守卫输出里 LaTeX 原文完整（没被切掉）',
    g.once.includes('\\left| TF_{1} \\right|=4,') && g.once.includes('F_{1}(-1,0)'),
    JSON.stringify(g.once).slice(0, 90),
  )
}

/* ============================================================
 * K：自动保存落盘的必须是**收尾后**的 Markdown + 被污染存档能自愈
 *
 *    真实事故（copy-test.txt 之谜的真正泄漏口，2026-09-26 复现成功）：
 *    onUpdate 曾把 storage.markdown.getMarkdown() 的**原始输出**（公式定界符
 *    还是 U+0002，finalizeMarkdown 没跑）交给 mdRef → 1.2s 后 persistDraft()
 *    原样写进存储。正常关页时 beforeunload 会用收尾版盖住（所以怎么测都测不出），
 *    但**崩溃 / 强杀 / 断电**时不会 —— 下次打开正文里全是"豆腐块" □、公式退化成
 *    纯文本，再存盘反斜杠又被 markdown 转义翻倍（\left → \\left）。
 *
 *    修法：① onUpdate 交收尾后的 getMarkdown ② persistDraft 出口过守卫
 *    ③ setMarkdown 载入时把成对标记还原成 $…$（老用户已被污染的存档自动救回）。
 * ============================================================ */
console.log('\n=== K. 自动保存落盘 + 被污染存档的自愈 ===')

const kStore = await page.evaluate(async () => {
  const ed = window.__EDITOR__
  window.__SET_MARKDOWN__(ed, '句中 $\\left| TF_{1} \\right|=4$ 收尾')
  // 敲真实内容触发 onUpdate → mdRef → 1.2s 后 persistDraft 落盘
  ed.chain().focus('end').insertContent('。').run()
  await new Promise((r) => setTimeout(r, 2200))
  try {
    const j = JSON.parse(localStorage.getItem('md-editor-docs-v1') || '{}')
    return (j.docs?.find((d) => d.id === j.currentId) || {}).md ?? ''
  } catch {
    return ''
  }
})
check(
  'K1 自动保存写进存储的没有控制字符（崩溃后重开不再是豆腐块）',
  kStore !== '' && !CTRL_RE.test(kStore),
  `命中 ${(kStore.match(CTRL_RE) || []).length} 个，存档 ${JSON.stringify(kStore).slice(0, 60)}`,
)
check('K2 自动保存写进存储的公式带 $ 定界符', /\$[^$\n]+\$/.test(kStore), JSON.stringify(kStore).slice(0, 60))
check(
  'K3 自动保存写进存储的反斜杠没翻倍',
  kStore.includes('\\left| TF_{1} \\right|=4') && !kStore.includes('\\\\left'),
  JSON.stringify(kStore).slice(0, 80),
)

/* 老版本崩溃时留下的"带标记的存档"：载入必须自愈（公式还原、豆腐块消失） */
const kHeal = await page.evaluate(() => {
  const ed = window.__EDITOR__
  const poisoned = '例一 \u0002F_{1}(-1,0)\u0002 满足 \u0002\\left| TF_{1} \\right|=4,\u0002 尾'
  window.__SET_MARKDOWN__(ed, poisoned)
  let math = 0
  ed.state.doc.descendants((n) => {
    if (n.type.name === 'mathInline' || n.type.name === 'mathBlock') math += 1
  })
  return { math, text: ed.state.doc.textContent, md: window.__MD__() }
})
check('K4 被污染的存档载入后公式自动还原成节点', kHeal.math === 2, `${kHeal.math} 个（应为 2）`)
check(
  'K5 载入后正文没有控制字符',
  !CTRL_RE.test(kHeal.text),
  `命中 ${(kHeal.text.match(CTRL_RE) || []).length} 个`,
)
check(
  'K6 自愈后再存盘：LaTeX 原样、反斜杠不翻倍',
  kHeal.md.includes('$\\left| TF_{1} \\right|=4,$') && !kHeal.md.includes('\\\\left'),
  JSON.stringify(kHeal.md).slice(0, 90),
)
