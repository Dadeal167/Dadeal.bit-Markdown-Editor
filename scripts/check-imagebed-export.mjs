...
/* ---------- 2. 助手不在、也没填令牌：不给文件，直接说清怎么办 ----------
   （用户要求：图片显示不出来的格式不要给 —— 以前会给一份内嵌图片的 .md，
     拿去知乎导入图片全丢，他连着踩了两次） */
const off = await exportWith(5198, TOKEN, 'no-assistant.md', { expectNoDownload: true })
console.log('\n--- 助手离线、也没令牌 ---')
check('助手离线：**没有下载文件**（不再给带不出图的 .md）', off.downloaded === null, `下载了 ${off.downloaded ?? '(没下载)'}`)
check('助手离线：提示里说清"先不导出"', /先不导出/.test(off.note), off.note.slice(0, 90))
check(
  '助手离线：提示给两条出路（开助手 / 填图床令牌）',
  /启动知乎助手/.test(off.note) && /图床设置/.test(off.note),
  off.note.slice(0, 130).replace(/\n/g, ' '),
)
...
async function exportWithToken({ fileName, mockOk }) {
  const page = await browser.newPage({ viewport: { width: 1280, height: 950 }, acceptDownloads: true })
  const errs = []
  const calls = []
  page.on('pageerror', (e) => errs.push(String(e).slice(0, 140)))
  ...
  await page.addInitScript(
    ([addrKey, tokenKey]) => {
      localStorage.setItem(addrKey, 'http://127.0.0.1:5198') // 故意指向没助手在的端口
      localStorage.setItem(tokenKey, 'whatever')
    },
    ['md-editor-zhihu-addr-v1', 'md-editor-zhihu-v1'],
  )
  await page.goto(app, { waitUntil: 'load', timeout: 60000 })
  await page.waitForSelector('.ProseMirror', { timeout: 30000 })
  await page.waitForTimeout(1200)
...
console.log('\n--- 不开助手 + 令牌没权限（应拦住、并说清原因）---')
const denied = await exportWithToken({ fileName: 'token-denied.md', mockOk: false })
check('令牌没权限：**不下载文件**（缺图的文件没意义）', denied.downloaded === null, `下载了 ${denied.downloaded ?? '(没下载)'}`)
check('令牌没权限：反馈里说清原因（权限不够）', /权限/.test(denied.note), denied.note.slice(0, 90).replace(/\n/g, ' '))
check(
  '令牌没权限：同时告诉两条出路（开助手 / 改令牌）',
  /启动知乎助手/.test(denied.note) && /图床设置/.test(denied.note),
  denied.note.slice(0, 130).replace(/\n/g, ' '),
)
