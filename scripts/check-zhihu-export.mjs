...
/* ---------- 追加：有本机图片但拿不到公开网址时，**不给文件**，而是说清怎么办 ----------
   用户要求：图片显示不出来的格式不要给（他连着两次拿到图进不了知乎的 .md）。
   所以这里断言"点了导出不会下载文件、并且提示里写清先开助手 / 填令牌"。 */
await page.keyboard.press('Escape')
await page.waitForTimeout(300)
...
check('有本机图片、又没上传条件时：**不下载文件**（不再给带不出图的 .md）', leaked === null, `下载了 ${leaked ?? '(没下载)'}`)
check('并且提示按"警告"样式显示', /result--bad/.test(note4Class), note4Class)
check(
  '提示里说清"先开助手 / 或填图床令牌"',
  /先不导出/.test(note4) && /启动知乎助手/.test(note4) && /图床设置/.test(note4),
  note4.slice(0, 120).replace(/\n/g, ' '),
)
