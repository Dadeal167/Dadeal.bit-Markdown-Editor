...
setlocal
rem --- This .bat lives in the launcher subfolder; the real payload (editor html,
rem     assistant/, runtime/) sits one level up. ROOT points there. ---
set "ROOT=%~dp0.."
cd /d "%ROOT%"
set "PORT=5174"
...
set "HTMLF="
for %%f in ("%ROOT%\\Dadealbit*.html") do if not defined HTMLF set "HTMLF=%%~nxf"
if not defined HTMLF goto :nohtml
if not exist "%ROOT%\\%HTMLF%" goto :nohtml
...
powershell -NoProfile -Command "Start-Process -FilePath '%NODE%' -ArgumentList 'assistant\\zhihu-assistant.mjs','--port','%PORT%' -WorkingDirectory '%ROOT%' -WindowStyle Hidden" >nul 2>nul
...
if exist "%APPDATA%\\Microsoft\\Windows\\Start Menu\\Programs\\Startup\\dadealbit-assistant.vbs" (
  powershell -NoProfile -Command "$s=Join-Path $env:APPDATA 'Microsoft\\Windows\\Start Menu\\Programs\\Startup\\dadealbit-assistant.vbs'; $t=[IO.File]::ReadAllText($s,[Text.Encoding]::Unicode); $d=(Resolve-Path '%ROOT%').Path.TrimEnd('\\'); if($t -like ('*'+$d+'*')){ exit 0 } else { exit 3 }" >nul 2>nul
  if errorlevel 3 (
...
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0_open-editor.ps1" -Html "%ROOT%\\%HTMLF%" -Token "%TOKEN%" -Port %PORT%
if errorlevel 1 goto :nobrowser
exit /b 0

:nobrowser
echo   [x] Could not hand the editor to a browser automatically.
echo       Just double-click the "Dadealbit*.html" file yourself (it sits in the folder
echo       one level up from this launcher folder) - the token is printed right here;
echo       paste it into the Zhihu panel if it asks.
echo   token: %TOKEN%
pause
exit /b 1
...
setlocal
rem --- launcher subfolder: the payload (assistant/, runtime/) is one level up ---
set "ROOT=%~dp0.."
cd /d "%ROOT%"
...
>> "%PS%" echo $root = Split-Path -Parent $dir
>> "%PS%" echo $node = Join-Path $root 'runtime\\node.exe'
...
>> "%PS%" echo Add-Content -Path $vbs2 -Encoding Unicode -Value ('sh.CurrentDirectory = ' + $q + $root + $q)
...
if defined SILENT (
  echo   Autostart entry updated to this folder: %ROOT%
  exit /b 0
)
...
echo.
echo   Now open the editor (the .html file in the folder one level up) - the Zhihu panel
echo   should show "assistant connected". If it does not, run the main launcher .bat instead.
echo.
...
/**
 * 把启动器写进交付文件夹的 **「启动器」子文件夹**。
 *
 * 为什么单开一个文件夹：根目录原来是"编辑器 + 说明 + 5 个 .bat + _open-editor.ps1"混在一起，
 * 用户一眼看不出该点哪个。现在根目录只留编辑器、说明和两个数据文件夹（assistant / runtime），
 * 所有 .bat 集中到「启动器」里；.bat 里用 %ROOT%（= %~dp0..）指回上一层。
 *
 * @param dir 交付文件夹（例如 桌面/自用）
 */
export function writeLauncherFiles(dir) {
  const launcher = resolve(dir, '启动器')
  ...
  for (const [name, text] of [
    ...
    writeFileSync(resolve(launcher, name), CRLF(text), 'ascii')
  }
  // 打开编辑器的小助手（PowerShell，同样纯 ASCII）：.bat 用 -File 调它
  writeFileSync(resolve(launcher, '_open-editor.ps1'), CRLF(OPEN_PS1), 'ascii')
  console.log('  ✓ 启动器/开始使用.bat（后台起助手 + 打开编辑器 + 自动带令牌）')
  console.log('  ✓ 启动器/停止助手.bat')
}
