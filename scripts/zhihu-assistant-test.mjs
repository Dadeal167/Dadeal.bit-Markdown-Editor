...
const waitUp = async () => {
  for (let i = 0; i < 40; i += 1) {
    try {
      const r = await fetch(`${BASE}/status?token=${TOKEN}`)
      if (r.ok) return true
    } catch {
      /* 还没起来 */
    }
...
try {
  ...
  // 第二次上传同一标题：应该更新同一篇草稿（知乎写作页就是当前草稿），而不是新增一篇
  console.log('\n再传一次（验证「更新同一篇草稿」）…')
  const second = await api('/draft', { title, html: html.replace('列表项一', '列表项一（已更新）') })
  check('重复上传仍是同一篇草稿', second.json?.ok === true && second.json?.title === title, String(second.json?.title))
