...
const IGNORE_CONSOLE = [/bili-user-fingerprint/, /report is not found/, /bilibili/i]
page.on('console', (m) => {
...
try {
  ...
  // 开启时：输入 "# " 应自动变成一级标题
  // 用编辑器 API 把光标放到文末（键盘 Control+End 在富文本里不可靠，会落到别的块里）
  await page.evaluate(() => window.__EDITOR__.commands.focus('end'))
  await page.keyboard.press('Enter')
  await page.keyboard.type('# 语法转换测试')
  await page.keyboard.press('Enter')
  await page.waitForTimeout(300)
  ...
  await page.evaluate(() => window.__EDITOR__.commands.focus('end'))
  await page.keyboard.press('Enter')
  await page.keyboard.type('# 关闭后测试')
  await page.keyboard.press('Enter')
  await page.waitForTimeout(300)
