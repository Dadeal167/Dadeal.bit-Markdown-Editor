...
try {
  ...
  await page.getByRole('button', { name: '存到草稿箱' }).click()
  console.log('  正在上传（纯文字约 40 秒；带图片时助手要等知乎上传，可能几分钟）…')
  // 有图时助手会等最多 100 秒看图片有没有出现，再加刷新校验，所以这里给足 7 分钟
  ...
  // 用助手接口核对一次：草稿里确实有这篇
  // 再传一次：标题仍然对得上，说明写的是同一篇草稿（新建的话标题会是空的）
  const verify = await fetch(`${BASE}/draft`, {
    method: 'POST',
    headers: { 'content-type': 'application/json', 'x-dadealbit-token': TOKEN },
    body: JSON.stringify({ title: docTitle, html: '<p>二次校验：更新同一篇草稿</p>' }),
  }).then((r) => r.json())
  check(
    '再次上传仍是同一篇草稿（更新而非新增）',
    verify?.title === docTitle,
    `第二次上传后草稿标题="${verify?.title}"`,
  )
