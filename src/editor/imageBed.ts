  ...
  for (const [i, src] of images.entries()) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        return { urls: out, warnings, error: friendlyError(res.status, body) }
      }
      const json = (await res.json()) as { content?: { path?: string } }
      const realPath = json.content?.path ?? path
      const picked = await pickUrl(realPath, repo, branch)
      if (!picked.verified) warnings.push('图片已上传，但公开地址暂时打不开（cdn 分发要几秒），稍后可重试')
      firstUrl = picked.url
      out.push(picked.url)
    } catch (e) {
      return { urls: out, warnings, error: '连不上 GitHub：' + String(e).slice(0, 120) }
    }
  }
