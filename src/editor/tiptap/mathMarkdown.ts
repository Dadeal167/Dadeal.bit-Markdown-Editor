...
export function finalizeMarkdown(md: string): string {
  ...
  const out = fixHardBreaks(withEscaped)
  ...
  return stripLeakedMarks(out, 'finalizeMarkdown')
}

/**
 * **标记字符泄漏的守门人** —— 导出 / 复制的结果里绝不允许出现控制字符。
 *
 * 为什么必须有（真实事故）：用户把编辑器内容 Ctrl+C 粘到记事本，拿到的文本里
 * **每个公式的定界符都变成了 U+0002**（他那边显示成"豆腐块" □，另一次粘贴里直接不可见，
 * 看起来就是"$ 全丢了、LaTeX 源码裸奔"）。逐字节取证他的文件：U+0002 出现 **332 次**，
 * 而那篇文档正好 **332 个 `$`** —— 精确吻合，说明 MATH_INLINE_MARK 流出去了。
 *
 * 平时 finalizeMarkdown 会把 `\u0002latex\u0002` 换成 `$latex$`，正常不该漏。
 * 但"不该漏"和"漏不了"是两回事：这条兜底把**任何**残留的控制字符清掉，
 * 而且**不依赖"我能复现那条路径"** —— 复现不了也挡得住。
 *
 * 清理策略：成对的标记按公式还原（公式不丢），剩下的孤立控制字符直接删掉
 *（它们在正文里没有任何正当用途）。清理时往控制台留一条痕迹：
 * 这是"本该不可能发生"的事，出过一次就该能被看到。
 */
export function stripLeakedMarks(md: string, where: string): string {
  if (!/[\u0000-\u0008\u000b\u000c\u000e-\u001f]/.test(md)) return md
  const before = md
  const out = md
    /* 成对的先按公式还原 —— 这样即使走到这里，公式也没丢 */
    .replace(
      new RegExp(`${MATH_INLINE_MARK}([^${MATH_INLINE_MARK}${MATH_BLOCK_MARK}]*?)${MATH_INLINE_MARK}`, 'g'),
      (_m, tex: string) => `$${tex}$`,
    )
    .replace(
      new RegExp(`${MATH_BLOCK_MARK}([^${MATH_INLINE_MARK}${MATH_BLOCK_MARK}]*?)${MATH_BLOCK_MARK}`, 'g'),
      (_m, tex: string) => `$$\n${tex}\n$$`,
    )
    /* 剩下的孤立控制字符一律删掉 */
    .replace(/[\u0000-\u0008\u000b\u000c\u000e-\u001f]/g, '')
  try {
    console.warn(`[公式标记守卫] ${where} 的输出里出现了控制字符，已清理（受影响 ${before.length - out.length} 字符）`)
  } catch {
    /* 控制台不可用就算了 */
  }
  return out
