...
export default function MathModal({ open, initialLatex, initialDisplay, onClose, onConfirm }: Props) {
  ...
  /** 候选列表变短时把选中项拉回范围内（点符号按钮会改 tex，列表跟着变短） */
  useEffect(() => {
    if (acIndex >= suggestions.length && acIndex !== 0) setAcIndex(0)
  }, [suggestions.length, acIndex])
              ...
              <textarea
                ...
                onKeyDown={(e) => {
                  ...
                  if (e.key === 'ArrowDown') {
                    ...
                    e.preventDefault()
                    // 列表可能刚变短（点了分类面板里的符号、或改了 tex），acIndex 可能越界 —— 兜一下，
                    // 否则 s 是 undefined，下面取 s.latex 直接抛 TypeError（这一下插入就失灵了）
                    const s = suggestions[acIndex] ?? suggestions[0]
