...
import { explainVideoInput, toEmbedUrl } from './tiptap/VideoNode'
import { escapeTablePipes } from './tiptap/tableMarkdown'
import {
  finalizeMarkdown,
  protectMathSpans,
  recoverEscapedMath,
  repairMathInMarkdown,
  stripLeakedMarks,
} from './tiptap/mathMarkdown'
import { repairLatex, rendersOk } from './math/latexRepair'
import { convertRawMath, countRawMath, escapeNonMathDollars, looksLikeLatex } from './math/rawMathText'
...
export function setMarkdown(editor: Editor, md: string): { fixedMath: number } {
  // 0) 先救"被内部标记污染过的存档"：以前的版本会把未收尾的序列化结果
  //    （公式定界符还是 U+0002/U+0003）自动保存进存储 —— 崩溃后重开，正文里
  //    全是"豆腐块" □、公式退化成纯文本。守卫把成对标记还原成 $…$（公式不丢）、
  //    孤立控制字符删掉，正常文档原样返回（开头就 return，不花钱）。
  md = stripLeakedMarks(md, 'setMarkdown 载入')
  // 顺序很重要：
  ...
  const { md: protectedMd, spans } = protectMathSpans(
    recoverEscapedMath(stripExporterHeader(md), looksLikeFormula),
  )
  let fixedMath = 0
...
/**
 * 公式体检，**两个数分开报**：
 *   stored    —— 节点里**存着**的写法就渲染不出来（需要修，多半修得好）
 *   unfixable —— 自动修过之后**仍然**渲染不出来（只能手动改）
 *
 * 为什么必须分开：以前只报"修过之后还坏不坏"这一个数，于是留下一个死角 ——
 * 像 `&lt;` 这种"存着是坏的、但 repairLatex 修得好"的公式会被数成 0，
 * 提示条不显示、那个真正会修的「一键修复公式」按钮也跟着不出现。
 * 结果是：屏幕上明明有红字，却找不到任何修复入口；
 * 菜单里那个「检查图片与公式」又只检查不修 —— 点十次也没用。
 * （实测：节点里存 `f'(x)&lt;0`，countBrokenMath 报 0，提示条不显示。）
 */
export function scanMathIssues(editor: Editor): { stored: number; unfixable: number } {
  let stored = 0
  let unfixable = 0
  for (const m of mathNodes(editor)) {
    if (!m.latex.trim()) continue
    if (!rendersOk(m.latex, m.display)) stored += 1
    if (!rendersOk(repairLatex(m.latex, m.display), m.display)) unfixable += 1
  }
  return { stored, unfixable }
}

/** 还有几个公式渲染不出来（修过之后仍然不行的，才算"修不了"） */
export function countBrokenMath(editor: Editor): number {
  return scanMathIssues(editor).unfixable
}

/**
 * 调试口：把"公式修复"这件事本身暴露出来，给自动化测试用。
 *
 * 为什么需要：安全网那条铁律（"修不好就一个字都不许改"）必须在**函数层面**验证 ——
 * 光看界面看不出"某段公式在被修复时有没有被改坏"。
 * 以前没有这个口，于是"修复反而破坏公式"这类 bug 一路绿灯跑到用户面前
 * （用户实测：点了修复公式之后，所有命令的反斜杠被吃掉，公式变成一堆源码）。
 * 只读、不改文档，暴露出去没有副作用。
 */
export function exposeRepairDebugHooks(): void {
  const w = window as unknown as Record<string, unknown>
  w.__REPAIR_LATEX__ = (tex: string, display = false) => repairLatex(String(tex ?? ''), !!display)
  w.__RENDERS_OK__ = (tex: string, display = false) => rendersOk(String(tex ?? ''), !!display)
  /* 导入管道的三段，逐步暴露 —— 用来定位"公式在哪一步失去了身份"。
     背景：源文件里的 `*`（本该是 `_`）会被 Markdown 解析器吃成 <em>，
     于是 $…$ 被切断、整段 LaTeX 变正文（用户截图：图2/图3）。
     光看界面无法判断是"没认出来"还是"认出来了又还原失败"，所以把每步都摊开。
     全部只读，不改文档。 */
  w.__LOOKS_LIKE_LATEX__ = (s: string) => looksLikeLatex(String(s ?? ''))
  w.__ESCAPE_NON_MATH_DOLLARS__ = (md: string) => escapeNonMathDollars(String(md ?? ''))
  w.__PROTECT_MATH_SPANS__ = (md: string) => {
    const { md: out, spans } = protectMathSpans(String(md ?? ''))
    return { md: out, spans }
  }
  /* 整条打开链（打开 .md / 初始文档都走它）—— 用来对"真实文件"逐步复现 */
  w.__SET_MARKDOWN__ = (editor: unknown, md: string) =>
    setMarkdown(editor as Editor, String(md ?? ''))
  /* 标记字符守卫本体：给它一段带控制字符的文本，看它能不能清干净、且不丢公式。
     为什么单独暴露：用户实测复制出来的文本里每个公式定界符都变成了 U+0002
     （显示成豆腐块 / 直接不可见），但我复现不出那条路径 ——
     那就直接测守卫本身，不依赖能不能复现。 */
  w.__STRIP_LEAKED_MARKS__ = (md: string, where = 'probe') => stripLeakedMarks(String(md ?? ''), where)
}
...
export async function prepareImageDataUrl(file: File): Promise<string> {
  ...
  try {
    ...
    // 把"读不出来"变成一句能看懂的话（FileReader 的原始报错对用户没意义）
    throw new Error(`图片读不出来（${file.name}）`, { cause: e })
  }
...
export function downloadText(name: string, content: string, type: string): void {
  ...
  a.href = url
  a.download = name
  // 必须先挂进文档再点：游离的 <a> 在某些浏览器上点不动（下载会静默失败）
...
export { explainVideoInput, toEmbedUrl }
