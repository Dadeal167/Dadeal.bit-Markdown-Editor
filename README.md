...
> 仓库里那几个 `.bat`（`开发模式.bat` / `重新构建.bat` / `启动知乎助手.bat`）是**给 clone 了仓库、装了 Node 依赖的人**用的；
...
4. 点「存到草稿箱」，等它跑完（**一般十几秒**：整篇一次交完，公式一百个也不慢；
   图片由知乎自己重新托管，图片多的时候多花几秒）。跑完面板会报「公式 N 个 · 图片 N/N 张」，
   数字对不上就别急着用，先去草稿箱看一眼
...
手动粘令牌也一样能用。**clone 仓库自己跑**的话：先 `pnpm install`，再双击仓库根目录的 `启动知乎助手.bat`。
...
1. **改动都要有验证**——仓库里有十几套自动化验证（共约 300 项断言），
   ...
   pnpm check:components   # 每个菜单/弹窗/面板：打得开、关得掉、不报错、不留蒙层 22 项
   pnpm check:symbols      # 386 个符号逐个渲染校验
   pnpm check:formula-import  # 导入 .md 后公式对不对 34 项
   pnpm check:image-import    # 相对路径图片能不能修好 10 项
   pnpm check:paste           # 粘贴进来的公式认不认 27 项
   pnpm check:zhihu-export    # 导出给知乎导入的 .md 对不对 36 项
   pnpm check:indexeddb       # 文档存储：迁移 / 大文档 / 写失败会不会重试 25 项
   pnpm check:codelang        # 代码块语言选择 31 项
   pnpm check:launcher        # 「开始使用.bat」真跑一遍（起助手→带令牌→停止）31 项
   pnpm check:assistant-inject # 助手往知乎正文放内容的手法没被改回坏写法 29 项
